...
This ROS 2 package, part of the `MonsterXPilot` codebase, provides a path planning solution for complete coverage of a defined area. It is suitable for applications like autonomous lawn mowing, field surveying, or cleaning robots. It generates efficient sweep-based paths while avoiding obstacles within a given boundary.
...
This project is designed to be set up as part of the `MonsterXPilot` workspace. The following instructions will guide you through setting up the complete development environment.
...
The `MonsterXPilot` repository includes a script to automate most of the setup process.

1. **Clone the main workspace repository:**

    ```bash
    git clone https://github.com/nontanan-linux/MonsterXPilot.git
    cd MonsterXPilot
    ```
...
1. **Build the packages:**
    From the root of your workspace (`~/MonsterXPilot`), run colcon:
...
```bash
source ~/MonsterXPilot/install/setup.bash
```
